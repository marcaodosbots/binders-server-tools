...
function runMigrations() {
    console.log('[db] verificando e aplicando migrações...');
...
function getUser(userId) {
    ...
    if (!user) { insertUser.run(userId); user = selectUser.get(userId); }
    return user;
}
function updateUser(userId, column, value) { db.prepare(`UPDATE users SET ${column} = ? WHERE userId = ?`).run(value, userId); }
function setLastKnownLocale(userId, locale) { updateUserLocale.run(locale, userId); }
...
function updateGuild(guildId, column, value) { db.prepare(`UPDATE guilds SET ${column} = ? WHERE guildId = ?`).run(value, guildId); }
