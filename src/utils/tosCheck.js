...
const getLanguage = require('./getLanguage.js'); // puxa nosso detetive

// central de textos pra esse handler
const texts = {
    welcome: {
        title: {
            'pt_BR': '<:novato:1394085774567276614> Termos de Serviço e Política de Privacidade',
            'en_US': '<:novato:1394085774567276614> Terms of Service & Privacy Policy'
        },
        description: {
            'pt_BR': 'Bem-vindo(a)! Para usar minhas funções, você precisa concordar com nossos Termos. Isso garante um ambiente seguro para todos.',
            'en_US': 'Welcome! To use my functions, you need to agree to our Terms. This ensures a safe environment for everyone.'
        }
    },
    update: {
        title: {
            'pt_BR': '<:novato:1394085774567276614> Termos de Serviço Atualizados!',
            'en_US': '<:novato:1394085774567276614> Terms of Service Updated!'
        },
        description: {
            'pt_BR': 'Nossos Termos foram atualizados! Para continuar, por favor, leia e aceite a nova versão.',
            'en_US': 'Our Terms have been updated! To continue, please read and accept the new version.'
        }
    },
    buttons: {
...
async function tosCheck(interaction) {
    ...
    // decide qual conjunto de textos usar (boas-vindas ou atualização)
    const textKey = isFirstTime ? 'welcome' : 'update';
    
    // monta o embed usando nossa fábrica e os textos centralizados
    ...
    // monta o payload da resposta, usando o jeito moderno de msg efêmera
    const payload = {
        embeds: [embed],
        components: [buttons],
    ...
    if (interaction.isButton()) {
        // se veio de um clique no botão, edita a msg original
        await interaction.update(payload);
    } else {
        // se veio de um comando de barra, cria uma resposta nova
